...
plt.plot(x[0:len(one_branch_pbe) - 8],
         one_branch_pbe[0:len(one_branch_pbe) - 8],
         '--',
         color='#A2142F',
         linewidth=lw)
plt.plot(x[0:len(selection_pbe) - 8],
         selection_pbe[0:len(selection_pbe) - 8],
         color='#0072BD',
         linewidth=lw)
plt.plot(x[0:len(mrc_pbe) - 8], mrc_pbe[0:len(mrc_pbe) - 8], color='#D95319',
         linewidth=lw)
plt.plot(x[0:len(crc_pbe) - 8], crc_pbe[0:len(crc_pbe) - 8], color='#4DBEEE',
         linewidth=lw)
plt.plot(x[0:len(egc_pbe) - 8], egc_pbe[0:len(egc_pbe) - 8], color='#EDB120',
         linewidth=lw)
plt.plot(x[0:len(rene_pbe) - 8],
         rene_pbe[0:len(rene_pbe) - 8],
         color='#7E2F8E',
         linewidth=lw)
plt.plot(x[0:len(power_and_crc_pbe) - 8],
         power_and_crc_pbe[0:len(power_and_crc_pbe) - 8], color='#77AC30',
         linewidth=lw)

plt.ylabel(' Payload score [-]')
plt.xlabel('Signal noise ratio (SNR) [dB]')
plt.grid(True)
plt.legend(['1 branch', 'Selection', 'MRC', 'CRC',
            'EGC', 'Differential', 'Power and CRC'])

plt.savefig('Payload_score_over_snr.pdf')
plt.show()
