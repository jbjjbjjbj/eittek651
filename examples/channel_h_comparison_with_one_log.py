...
data_formodlling = rssi_str[200:400]
x_value_modeling = time_from_zero[200:400]
...
ch_2 = channel_models.RayleighAWGNChannel(
    1, 0, frame_per_block=3, intermediate_point=False)
...
axs[0].plot(x_value_modeling, h_1, linewidth=1,
            linestyle='-', color='#0072BD')

axs[0].plot(x_value_modeling, data_formodlling_norm, linewidth=1,
            linestyle='--', color='#A2142F')

axs[1].plot(x_value_modeling, h_2, linewidth=1,
            linestyle='-', color='#77AC30')

axs[1].plot(x_value_modeling, data_formodlling_norm, linewidth=1,
            linestyle='--', color='#A2142F')

axs[2].plot(x_value_modeling, h_3, linewidth=1,
            linestyle='-', color='#EDB120')

axs[2].plot(x_value_modeling, data_formodlling_norm, linewidth=1,
            linestyle='--', color='#A2142F')

axs[3].plot(x_value_modeling, h_old, linewidth=1,
            linestyle='-', color='#7E2F8E')

axs[3].plot(x_value_modeling, data_formodlling_norm, linewidth=1,
            linestyle='--', color='#A2142F')
# print('sigma',sigam_square)

axs[0].legend(['Channel h with intermediate',
              'Library data'], fontsize='xx-small', loc='lower right')
axs[1].legend(['Channel h without intermediate',
              'Library data'], fontsize='xx-small', loc='lower right')
axs[2].legend(['Channel h with intermediate 4 frame Tc',
              'Library data'],
              fontsize='xx-small', loc='lower right')
axs[3].legend(['Old channel h', 'Library data'],
              fontsize='xx-small', loc='lower right')


plt.xlabel('Time [ms]')
plt.ylabel('RSSI [dBm]')
plt.savefig('New_old_channel_model_with_lib_data.pdf')
plt.show()
